...
class marketFrontend {
    ...
    private function accountIsAuthenticated() {
        ...
        if ($this->use_lepton_auth && $wb->is_authenticated()) {
            ...
            if (! isset($_SESSION['GROUPS_ID']) || empty($this->params[self::param_lepton_groups])) {
                $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, market_error_lepton_group_missing));
                return false;
            }
            ...
            $market_groups = explode(',', $this->params[self::param_lepton_groups]);
            foreach ($lepton_groups as $lepton_group) {
                if (in_array($lepton_group, $market_groups)) {
                    ...
                    if (! $kitContactInterface->isEMailRegistered($email, $contact_id, $status)) {
                        ...
                        $contact_array[kitContactInterface::kit_intern] = $dbMarketCfg->getValue(dbMarketCfg::cfgKITcategory);
                        $register = array();
                        // insert contact
                        ...
                        $data = array(
                                ...
                                dbKITregister::field_update_by => 'kitMarketPlace');
    ...
    public function accountAdvertisementAdd() {
        ...
        if ($ad_id > 0) {
            ...
            $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' AND %s!='%s'", $dbMarketAd->getTableName(), dbMarketAdvertisement::field_id, $ad_id, dbMarketAdvertisement::field_status, dbMarketAdvertisement::status_deleted);
            $ad = array();
        ...
        $category_list = array();
        $category_list[] = array('text' => market_text_select, 'value' => - 1, 
        ...
        foreach ($categories as $category) {
            ...
            if (! in_array($path_str, $category_check)) {
                ...
                $category_list[] = array('text' => $path_str, 
        ...
        foreach ($dbMarketAd->type_array as $value => $text) {
            $ad_types[] = array('text' => $text, 'value' => $value, 
        ...
        foreach ($dbMarketAd->commercial_array as $value => $text) {
            $commercial_array[] = array('text' => $text, 'value' => $value, 
        ...
        foreach ($dbMarketAd->price_array as $value => $text) {
            $price_types[] = array('text' => $text, 'value' => $value, 
            'selected' => ($value == $ad[dbMarketAdvertisement::field_price_type]) ? 1 : 0);
        }
        
        $form = array('name' => 'advertisement_add', 
        'action' => array('name' => self::request_main_action, 
        'value' => self::action_account, 'link' => $this->page_link), 
        ...
        'value' => $ad_id), 'title' => market_head_advertisement_add, 
        'response' => array(
        'text' => ($this->isMessage()) ? $this->getMessage() : market_intro_advertisement_add, 
        ...
        'values' => $category_list, 'hint' => market_hint_adv_category), 
        'type' => array('label' => market_label_adv_type, 
        'name' => dbMarketAdvertisement::field_ad_type, 'values' => $ad_types, 
        'hint' => market_hint_adv_type), 
        'commercial' => array('label' => market_label_adv_commercial, 
        'name' => dbMarketAdvertisement::field_commercial, 
        'values' => $commercial_array, 'hint' => market_hint_adv_commercial), 
        ...
        'values' => $price_types, 'hint' => market_hint_adv_price_type), 
        ...
        'value' => array(
        'closed' => array('value' => dbMarketAdvertisement::status_closed, 
            ...
            if ($_FILES[dbMarketAdvertisement::field_pictures]['error'] == UPLOAD_ERR_OK) {
                ...
                if (! in_array($ext, $allowed_filetypes)) {
                    ...
                    if (($origin_width > $max_width) || ($origin_height > $max_height)) {
                        ...
                        if (false === ($new_file = $this->createTweakedFile($path_parts['filename'], $ext, $upl_file, $new_width, $new_height, $origin_width, $origin_height))) {
    ...
    public function accountOverview() {
        ...
        $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' AND %s!='%s' ORDER BY %s DESC", $dbMarketAd->getTableName(), dbMarketAdvertisement::field_kit_id, $_SESSION[kitContactInterface::session_kit_contact_id], dbMarketAdvertisement::field_status, dbMarketAdvertisement::status_deleted, dbMarketAdvertisement::field_timestamp);
        $ads = array();
        ...
        if (count($ads) < 1) {
            ...
            foreach ($ads as $ad) {
                $cat = array();
                $where = array(
                dbMarketCategories::field_id => $ad[dbMarketAdvertisement::field_category]);
                $category = array();
    ...
    public function marketAction() {
        ...
        $data = array('content' => $content, 'category' => $cat, 
    ...
    public function marketCategoriesList() {
        ...
        foreach ($categories as $cat) {
            ...
            'id' => $cat[dbMarketCategories::field_id], 'text' => $text, 
            'link' => sprintf('%s%s%s=%s&%s=%s', $this->page_link, (strpos($this->page_link, '?') === false) ? '?' : '&', self::request_market_action, self::action_category, dbMarketCategories::field_id, $cat[dbMarketCategories::field_id]), 
            'level' => $level, 
    ...
    public function marketShowCategory(&$cat_id = -1) {
        ...
        if ($cat_id < 1) {
            ...
            $SQL = sprintf("SELECT * FROM %s,%s WHERE %s=%s AND %s='%s' ORDER BY %s DESC LIMIT %s", $dbMarketAd->getTableName(), $dbMarketCats->getTableName(), dbMarketAdvertisement::field_category, dbMarketCategories::field_id, dbMarketAdvertisement::field_status, dbMarketAdvertisement::status_active, dbMarketAdvertisement::field_timestamp, $max_entries);
        } else {
            ...
            $SQL = sprintf("SELECT * FROM %s,%s WHERE %s=%s AND %s='%s' AND %s ORDER BY %s DESC", $dbMarketAd->getTableName(), $dbMarketCats->getTableName(), dbMarketAdvertisement::field_category, dbMarketCategories::field_id, dbMarketAdvertisement::field_status, dbMarketAdvertisement::status_active, $fields, dbMarketAdvertisement::field_timestamp);
        }
        ...
        foreach ($advertisements as $ad) {
            ...
            $items[] = array('id' => $ad[dbMarketAdvertisement::field_id], 
            'images' => $images, 'category' => $kat_str, 
            ...
            'link' => sprintf('%s%s%s=%s&%s=%s', $this->page_link, (strpos($this->page_link, '?') === false) ? '?' : '&', self::request_market_action, self::action_advertisement, dbMarketAdvertisement::field_id, $ad[dbMarketAdvertisement::field_id]), 
            'contact' => $contact);
        }
        
        $data = array('category' => $cat_id, 'items' => $items);
    ...
    public function marketShowAdvertisement() {
        ...
        $item = array('id' => $ad[dbMarketAdvertisement::field_id], 
        'images' => $images, 'category' => $kat_str, 
        'type' => $dbMarketAd->type_array[$ad[dbMarketAdvertisement::field_ad_type]], 
        'commercial' => $dbMarketAd->commercial_array[$ad[dbMarketAdvertisement::field_commercial]], 
        'title' => $ad[dbMarketAdvertisement::field_title], 
